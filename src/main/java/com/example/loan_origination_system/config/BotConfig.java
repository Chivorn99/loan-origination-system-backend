...
public class BotConfig {
    ...
    public TelegramBotsApi telegramBotsApi(PawnShopTelegramBot pawnShopTelegramBot) {
        try {
            // This is the missing link! It creates the session and plugs your bot into the matrix.
            TelegramBotsApi api = new TelegramBotsApi(DefaultBotSession.class);
            api.registerBot(pawnShopTelegramBot);
            System.out.println("✅ Telegram Bot successfully connected to the internet!");
            return api;
        } catch (TelegramApiException e) {
            System.err.println("❌ Failed to connect Telegram Bot: " + e.getMessage());
            return null;
        }
    }
}